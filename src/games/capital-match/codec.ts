 ...
 *
 * The digit range is the four capitals a question always offers. It is spelled out here
 * rather than imported from module.ts: the play page loads the codec on its own (the server
 * decodes the cookie without the board), so this file must not pull the module in, and a
 * value import would close a cycle that throws before the module is initialised.
 */
const OPTIONS = 4;
...
export const codec: Codec<CapitalAction> = {
  ...
  dec(s) {
    return [...s].map((ch) => {
      const i = Number(ch);
      if (!Number.isInteger(i) || i < 0 || i >= OPTIONS) throw new Error(`capital-match: bad answer "${ch}"`);
