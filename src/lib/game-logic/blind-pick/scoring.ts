import type { BlindPickGame, BlindPickResult, BlindPickAssignment } from "./types";

export function computeResult(state: BlindPickGame): BlindPickResult {
  const { config, assignments } = state;
  const playerAssignments: BlindPickAssignment[] = [];
  let playerScore = 0;
  ...
  const optimalAssignments: BlindPickAssignment[] = config.optimalAssignment.map(
    (catIdx, countryIdx) => ({
      countryIdx,
      categoryIdx: catIdx,
...
function gapToGrade(gap: number): BlindPickResult["grade"] {
